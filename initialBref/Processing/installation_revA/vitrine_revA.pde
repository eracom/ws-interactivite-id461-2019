...
void setup() {
  size(1024, 768); //taille de la fenêtre
  //fullScreen();
  //noCursor();
...
void display() {
  ...
  processCursor = int(map(Cursor,830,12000,0,255)); // modification de la valeur donnée par le capteur 
...
void checkFile() { //fonction de comptage d'image
  dirImages = new File(sketchPath+"/data/images/");
...
void loadFile() { //fonction de chargement des images
  for (int i = 0; i<imageCount; i++) {
    listeDimages.add(loadImage("images/cube"+nf(i, 4)+".png"));   // charger les images du dossier images se nommant left + normalisation à 4 numero + .png
